<style>
    ...
    .text_info {
        font-weight: normal;
        margin-top: 10px;    /* ระยะห่างด้านบนของข้อความ */
    margin-bottom: 10px; /* ระยะห่างด้านล่างของข้อความ */
    padding: 10px;       /* ระยะห่างด้านในของคอนเทนเนอร์ */
    ...
    <div class="row box_detail ">
        <div class="col-12">
            <div class="row">
                <div class="col-6 text-content ml-2">
                    <p style="font-size: 20px; font-weight: bold;">Order Code : {{ $item->code_order }}</p>

                    <p >Name recipient : {{ $item->customers_name }}</p>

                    @foreach ($item->address as $address)
                            <span class="text_head">Delivery address :
                                <span class="text_info"> {{ $item->customers_name }} </span>
                                <span class="text_info"> {{ $address->house_no }}</span>
                                <span class="text_info">Moo {{ $address->moo }}</span>
                                <br>  <br>
                                <span class="text_info">Soi {{ $address->soi }}</span>
                                <span class="text_info">Road {{ $address->road }}</span>
                                <span class="text_info">Tambon {{ $address->tambon }}</span>
                                <span class="text_info">District {{ $address->district }}</span>
                                <span class="text_info"> {{ $address->province }} {{ $address->zipcode }}</span>
                            </span>
                        </p>
                    @endforeach

                    {{-- <p style="margin-top: 50px"> Product list</p> --}}
                </div>
                ...
                <div class="col-4 text-content mt-4 ml-3" >
                    ...
                    <p>Order date : {{ $date }} </p>
                    @foreach ($item->address as $address)
                        <p>Phone number : {{ $address->tel }}</p>
                    @endforeach
                    <p style="font-size: 20px; font-weight: bold;">  Transportation service {{ $item->tracking_type }} </p>
                </div>
        ...
        <div class="row text-total text-content">
            ...
            <div class="col-2 mr-2  text-right  ">
                ...
                {{-- <p>{{ number_format($item->discount) }} USD</p> --}}
                <p>{{ number_format($item->total_price) }} บาท</p>
            </div>
        </div>
